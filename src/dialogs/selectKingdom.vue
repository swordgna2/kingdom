<template>
    <div>
        ...
        <section>
            ...
            <p><label>
                <select v-if="options" v-model="selected">
                    <option v-for="(option, index) in options" v-bind:key="index" v-bind:value="index">
                        {{ option }}
                    </option>
                </select>
            </label>&nbsp;<button name="random" v-on:click="flushKingdom">случайная</button></p>
        ...
        <footer>
            <button name="proceed">Продолжить</button>
        </footer>
    </div>
</template>
